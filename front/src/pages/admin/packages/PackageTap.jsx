  ...
  return (
    <div>
      ...
      <section className="mt-[3rem]">
        <div>{movie?.review?.length === 0 && <p>No Reviews</p>}</div>

        <div>
          {movie?.review?.map((reviewk) => (
            <div
              key={reviewk._id}
              className="bg-[#1A1A1A] p-4 rounded-lg w-[50%] mt-[2rem]"
            >
              <div className="flex justify-between">
                <strong className="text-[#B0B0B0]">{reviewk.name}</strong>
                <p className="text-[#B0B0B0]">
                  {reviewk.createdAt}
                </p>
              </div>

              <p className="my-4">{reviewk.comment}</p>
            </div>
          ))}
        </div>
      </section>
